...
(defn- closer [bracket] ; get the character under the cursor
  ...
  (each [k v (pairs getCursor)]
    (match k ; store column and row to vars
      ...
      2
      (set col v)))
  (def currLine (vim.api.nvim_get_current_line)) ; get current line
  (def currCharacter (currLine:sub (+ col 1) (+ col 1))) ; get current character
  (if (= (tostring bracket) (tostring currCharacter)) ; if current character is the bracket then return <Right>
      (do ; we need to use the termcode directly, else <Right> prints literally
        (set result :<Right>))
      (do

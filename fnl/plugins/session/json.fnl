...
(defn encode [session] "Encode lua table as a json"
  (vim.json.encode session))

(defn decode [json] "Decode json into a lua table"
  (vim.json.decode json))

(defn ->file! [json] "Store json to file"
  (os.execute (.. "rm " stored-file))

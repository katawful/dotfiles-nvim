...
(defn fennel-lua []
  "Download fennel.lua into 'fnl/plugins/fnlfmt' with curl"
  (vim.fn.system 
      "curl -o ~/.config/nvim/fnl/fennel.lua \\
'https://git.sr.ht/~technomancy/fnlfmt/blob/main/fennel.lua'")
  (if (not= vim.v.shell_error 0)
    (vim.notify "Curl error, could not download fennel.lua" vim.log.levels.ERROR)
    (vim.fn.system (.. "cp " 
